...
EVENT_IN = "market.liquidity.updated"
EVENT_DAY = "SYS_DAY"
EVENT_PULSE = "SYS_SECOND"
EVENT_OUT = "storage.liquidity_saved"
...
_INDEXES = (
    "CREATE INDEX IF NOT EXISTS idx_liquidity_symbol ON liquidity(symbol, id DESC)",
    "CREATE INDEX IF NOT EXISTS idx_liquidity_time ON liquidity(occurred_at)",
)

_INSERT = "INSERT INTO liquidity (symbol, occurred_at, payload_json) VALUES (?,?,?)"

_PRUNE = "DELETE FROM liquidity WHERE occurred_at IS NOT NULL AND occurred_at < ?"
    ...
    async def _flush_and_report(
        ...
        try:
            self._limit_state = await asyncio.to_thread(
                enforce_limits, self._db_path, "liquidity",
