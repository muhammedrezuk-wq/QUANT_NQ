...
_INDEXES = (
    ...
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_dedupe ON orders(account_id, request_id) WHERE request_id IS NOT NULL",
)

_INSERT = (
    "INSERT OR IGNORE INTO orders (request_id, account_id, symbol, side, volume, occurred_at, payload_json)"
    " VALUES (?,?,?,?,?,?,?)"
)
    ...
    def _ensure_store(self) -> bool:
        try:
            ...
            try:
                ...
                columns = {str(row[1]) for row in connection.execute("PRAGMA table_info(orders)")}
                if "request_id" not in columns: connection.execute("ALTER TABLE orders ADD COLUMN request_id TEXT")
    ...
    async def _flush_and_report(
        ...
        try:
            self._limit_state = await asyncio.to_thread(
                enforce_limits, self._db_path, "orders",
    ...
    async def health_check(self) -> HealthStatus:
        ...
        if self.stored_count == 0 and not self._buffer:
            if self.dropped_count:
                return HealthStatus(
                    state=HealthState.DEGRADED,
                    message="SUSPECTED_FAULT: %d orders dropped and nothing stored" % self.dropped_count,
                    details=details)
            return HealthStatus(
                state=HealthState.HEALTHY,
                message="READY_AWAITING_FIRST_ORDER_STORE | stored=0",
                details=details)
        return HealthStatus(
            state=HealthState.HEALTHY,
