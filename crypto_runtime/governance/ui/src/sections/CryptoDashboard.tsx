...
const num = (value: unknown, digits = 2): string => { const n = Number(value); return Number.isFinite(n) ? n.toLocaleString('en-US', { maximumFractionDigits: digits }) : '—' }
...
function DecisionCard({ card }: { card: AnyMap }) {
  return <div className="crypto-decision-card">
    <div className="crypto-decision-head"><b>ذرّة 456</b><span>{text(card.symbol)} · {text(card.account_id, 'بلا حساب')}</span></div>
    <div className="crypto-decision-grid">
      <div><span>الحالة</span><b>{text(card.state ?? card.status)}</b></div>
      <div><span>الاتجاه</span><b>{text(card.direction ?? card.side)}</b></div>
      <div><span>دخول</span><b>{num(card.entry ?? card.entry_price, 6)}</b></div>
      <div><span>وقف</span><b>{num(card.stop ?? card.stop_price, 6)}</b></div>
      <div><span>هدف 1</span><b>{num(card.target1 ?? card.take_profit_1, 6)}</b></div>
      <div><span>هدف 2</span><b>{num(card.target2 ?? card.take_profit_2, 6)}</b></div>
    </div>
    <small>{text(card.reminder ?? card.reason, 'بيانات عرض فقط — لا تنفيذ آلي')}</small>
  </div>
}
...
export default function CryptoDashboard() {
  ...
  const triggerRows = useMemo(() => Object.entries(streams).filter(([name]) => /trigger|veto|gate/i.test(name) && !name.includes('feed')), [streams])
  const decisionCard = useMemo(() => Object.entries(streams).find(([name, value]) => /decision\.card|decision\.state|crypto\.card/i.test(name) && asMap(value).symbol), [streams])
  ...
  return <section className="crypto-root" dir="rtl">
    ...
    {tab === 'overview' && <>
      ...
      <div className="crypto-panel"><div className="crypto-panel-head"><h2>البطاقة الحيّة</h2><span>تظهر فقط عند وصول بطاقة فعلية</span></div>{decisionCard ? <DecisionCard card={asMap(decisionCard[1])} /> : <Empty>لا توجد بطاقة قرار فعلية الآن. لا توجد بطاقة مختلَقة.</Empty>}</div>
    ...
    {tab === 'trigger' && <div className="crypto-panel"><div className="crypto-panel-head"><h2>محكمة الزناد — ذرّة 455</h2><span>فيتو AND · لا تنفيذ من اللوحة</span></div>{triggerRows.length ? <div className="crypto-list">{triggerRows.map(([name, value]) => <div className="crypto-list-row" key={name}><b>{labelFor(name)}</b><span>{JSON.stringify(value)}</span></div>)}</div> : <Empty>لا يوجد قرار زناد فعلي الآن. لا يوجد «عبرت» مختلَق.</Empty>}<div className="crypto-policy">المسموح: تحديث أو تأكيد تنبيه. الممنوع دائمًا: شراء، بيع، تنفيذ آلي، ومفتاح تداول.</div></div>}
