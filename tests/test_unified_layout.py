...
def test_complete_release_contains_forex_and_full_crypto_source() -> None:
    ...
    assert len(crypto.atoms) == 84  # 2026-08-30: دمج الشريك +8 (2270-2277) وإزالة 2160
    assert {1001, 1002}.issubset({a.manifest.id for a in crypto.atoms})
    assert len({a.manifest.id for a in crypto.atoms}) == 84  # 2026-08-30: مع 2270-2277 وبلا 2160
