...
class Bootloader:
    ...
    async def boot(self, include_ids: set[int] | None = None) -> BootReport:
        ...
        for aid in sorted(working):
            mode = getattr(working[aid].manifest.startup_mode, "value", "auto")
            if mode != "auto":
                _log.info("الذرة %s startup_mode=%s — تُكتشف ولا تُشغَّل تلقائيًا (المادة 21)", aid, mode)
                excluded.append(aid)
                del working[aid]

        boot_order = self._resolve_with_graceful_degradation(working, excluded, critical_failures)
        selected_ids = set(working) if include_ids is None else (set(include_ids) & set(working))

        for atom_id in boot_order:
            if atom_id not in selected_ids or atom_id in self._registry:
                continue
        ...
        failed: list[int] = []
        for atom_id in boot_order:
            if atom_id not in selected_ids or atom_id not in self._registry:
                continue
    ...
    @staticmethod
    def atom_module_prefix(atom_id: int) -> str:
        """بادئة موديولات الذرة في `sys.modules` — مُعرَّفة مرّة، يستعملها التسجيل
        (هنا) والتطهير (hot_reload) معًا، فلا ينكسر التنظيف بصمت لو تغيّر التنسيق
        مستقبلًا (فتحة النواة V2.0 — ورقة ٠٦)."""
        return f"_atom_{atom_id}_"
    ...
    def instantiate(discovered: DiscoveredAtom) -> AtomBase:
        ...
        unique_name = f"{Bootloader.atom_module_prefix(discovered.manifest.id)}{module_name}"
    ...
    async def _initialize_and_start(self, atom_id: int) -> bool:
        ...
        context = AtomContext(
            ...
            subscribe_all=lambda handler, _aid=atom_id: self._event_bus.subscribe_all(
    ...
    def _isolate_sibling_modules(atom_id: int, atom_dir: str, before: set[str]) -> None:
        ...
        for name in set(sys.modules) - before:
            ...
            try:
                ...
                continue
            if same_dir:
                sys.modules[f"{Bootloader.atom_module_prefix(atom_id)}sib_{name}"] = mod
