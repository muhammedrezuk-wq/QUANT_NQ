"""Buy-side eligibility checker inside the single decision path.

NQ seal item 22, batch B (B2 per paper Q8, dials B3 per verdict Q6):
455 receives the SAME decision cycle as everyone else -- the output of 453
...
Q6 thresholds are governed dials (shared/decision_dials.py): this atom owns
DECISION_BUY_MIN_DIRECTION and re-reads the shared floors (owned by 457)
through effective_value() at every cycle, so an owner approval takes effect
from the next decision cycle without restarts.
"""
...
EVENT_OUT = "decision.eligibility.buy.state"
...
# this wiring: 276 cycles, all FIELD_UNKNOWN:direction, because the word was
...
ID_ELIGIBILITY = "buy_eligibility"
SIDE = "buy"
...
REASON_DIRECTION = "DIRECTION_BELOW_THRESHOLD"
...
#: DECISION_BUY_MIN_DIRECTION is owned here (commands applied by 455);
...
DIALS_OWNED = {"DECISION_BUY_MIN_DIRECTION": "_buy_min_direction"}
...
class Atom(AtomBase):
    ...
    async def _publish_dials_state(self) -> None:
        ...
        await self._context.publish(EVENT_DIALS_STATE, {
            "id": "decision_dials_455", "atom_id": ATOM_ID, "status": STATUS_OK,
            "dials": {"DECISION_BUY_MIN_DIRECTION": self._buy_min_direction,
    ...
    def _evaluate(self, view: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
        """Q6 buy-side conditions over the eight arriving fields. Every
        ...
        passed = direction is not None and direction >= self._buy_min_direction
        checks.append(_check("direction", direction, self._buy_min_direction, passed))
        if not passed:
