...
#
# ٢٠٢٦-٠٩-٠٦ (تصحيح ثانٍ، مقيس): الأرضية 20.00 قِيست على الوقف
# **المُرسل** — هو ما ينزلق عند الوسيط. لكنّها طُبِّقت على وقف
# **التحليل**، بينما `cost_pad` أدناه يضيف فوقه ثلاثة سبريدات (15.00)
# قبل الإرسال. فحُوسبت الحماية مرّتين: وقف تحليل 19.97 يصل الوسيط عند
# 34.97، ويطلب هدفًا على بعد 29.95 — بينما مدى خريطة المستويات كلّها
# 27.80 (مقيس: n_below=8 span_below=27.80 · n_above=1 span_above=0.93،
# ولم تتعمّق في خمس دقائق). النتيجة: NO_STRUCTURE_LEVEL دائم، لا لعطب
# بل لاستحالة حسابية صنعتُها.
#
# الأرضية تعود إلى موضعها المقيس — على المسافة المرسلة — ويُطرح منها
# ما يضيفه cost_pad. فوقف تحليل 5.00 يصل الوسيط عند 20.00: نفس الحماية
# المقيسة بالضبط، ووقفٌ بنيويّ حقيقيّ بدل رقم مصنوع.
#
# ٢٠٢٦-٠٩-٠٦ (تصحيح ثالث — كسر سقف المالك مقيسًا): التذكرة 1911353370
# خسرت 125.07$ فوق سقف المئة، لأن التنفيذ انزلق 20.9 نقطة على وقف
# مرسل 24.25. والجدول يقول إن الذنب ليس الانزلاق بل **مكان** الوقف:
#     مسافة الوقف المرسل   أقصى انزلاق مقيس (٣٠ صفقة)
#     ٠ → ١٥                25.06
#     ١٥ → ٢٥               20.90   ← هنا أوقعتها الأرضية 20.00
#     ٢٥ → ٣٥                1.11
#     ٣٥ +                   9.16
# أرضية العشرين كانت تضع كل وقف في أسوأ نطاق. الثلاثون تضعه في أهدأه،
# فيبقى اللوت كبيرًا (المخاطرة لا تُقتطع) وتنكمش الخسارة الواقعة.
#
# ٢٠٢٦-٠٩-٠٦ (تصحيح رابع وأخير — بعد نقل ملكية الصفقة): الأرضية أعلاه
# **حُذفت من الحساب**. الجدول أعلاه صحيح ويبقى للتاريخ، لكن موضعه خطأ:
# حين صار الإبطال ملك الاستراتيجية لم يعد لأرضيةٍ اخترعناها أن تحكم على
# فكرة مالكها — وقد قِيس أثرها: TARGET_INSIDE_BROKER_MIN يرفض كل إعداد
# لأن حدًّا قدره 14.89 لا علاقة له بالوسيط كان يُطبَّق على الهدف أيضًا.
# حماية الانزلاق انتقلت إلى موضعها الصحيح: `cost_pad` يزيح الوقف المرسل
# ثلاثة سبريدات، و`TAIL_SLIPPAGE_FRAC` في 551 يقيّد الخسارة الواقعة تحت
# سقف المالك. المال يُحمى بالتحجيم، لا بقتل الفكرة.
...
async def request_orders(atom: Any, scope_key: str, out: dict[str, Any]) -> None:
    ...
    for side, field in ((BUY, "delta_buy"), (SELL, "delta_sell")):
        ...
        # ما سيُضاف إلى الوقف قبل إرساله (يُحسب أدناه بالقيمة نفسها).
        # الأرضية المقيسة على المسافة المرسلة تُطرح منه، فلا تُحتسب
        # الحماية مرّتين ويبقى الوقف الواصل إلى الوسيط عند حدّه المقيس.
        # ٢٠٢٦-٠٩-٠٦ (مقيس — TARGET_INSIDE_BROKER_MIN يرفض كل فكرة):
        # كان `min_gap` يخلط شيئين مختلفين: **حدّ الوسيط** (قيد فيزيائيّ
        # حقيقيّ) و**أرضية انزلاق** اخترعتُها للمعمارية القديمة حين كان
        # 581 يؤلّف الوقف. القياس: الوسيط يعلن stops_level = 0، وكل رفض
        # RETCODE_10016 وقع بمسافة دون سبريد واحد. فحدّه الحقيقيّ سبريد،
        # لا 14.89 — والأخيرة كانت تقتل أهداف الأفكار جميعًا.
        # الحدّ صار حدّ الوسيط وحده. وحماية المال لم تنقص: `cost_pad`
        # يزيح الوقف المرسل ثلاثة سبريدات، وبدل الذيل في 551 يقيّد الخسارة
        # الواقعة تحت المئة — أي أن المال محميّ بالتحجيم لا بقتل الفكرة.
        min_gap = max(
        ...
        # صفقة): كان حدّ الوسيط **يرفض فكرة المالك** لأن إبطالها أضيق
        # منه. وهذا خطأ معماريّ: الحدّ قيدٌ ميكانيكيّ عند الوسيط، لا حكمٌ
        # على الفكرة. ورقة التنفيذ §٧ تفصل الاثنين — والإبطال التحليليّ
        # يبقى كما هو بينما **وقف التنفيذ** يتمدّد للخارج ليصير مقبولًا.
        # الفكرة لا تُرفض لأسباب ميكانيكية أبدًا؛ التنفيذ يتكيّف معها.
        if min_gap > 0 and abs(price - stop_loss) < min_gap:
            stop_loss = (price - min_gap) if side == BUY else (price + min_gap)
            shifted = True
            risk_gap = min_gap
        if min_gap > 0 and abs(take_profit - price) < min_gap:
            # الهدف لا يُزاح — هو معنى الفكرة. هدفٌ أقرب من حدّ الوسيط
            # يعني أن الفكرة نفسها لا تتّسع للتنفيذ، فتُترك بلا تزوير.
            atom._context.logger.warning(
                "581 لا صفقة %s: TARGET_INSIDE_BROKER_MIN — هدف الفكرة %s "
                "أقرب من حدّ الوسيط %.2f", symbol, take_profit, min_gap)
            continue
